...
def u_func(x):
    return V0 * eval_laguerre(5, abs(x)) if abs(x) < L else W
...
def plot_wave_functions(arg_energies, arg_wave_functions):
    ...
    for i, psi in enumerate(arg_wave_functions):
        ...
        Psi_norm = normalize_wave_function_b(Psi.copy())
...
def normalize_wave_function_b(y):
...
def f_fun(e, n_arg):
    F = np.array([c * q(e, X[i]) for i in np.arange(n_arg)])
    Psi[0] = 0.0
    Fi[n_arg - 1] = 0.0
    Psi[1] = d1
    Fi[n_arg - 2] = d2
    ...
    return derivative_func(Psi, h, r) - derivative_func(Fi, h, r)
...
print(f"Exact energies: {exact_energies}")
...
print(f"Wave functions: {wave_functions}")
normalized_wave_functions = [normalize_wave_function(psi) for psi in wave_functions]
print(f"Normalized Wave functions: {normalized_wave_functions}")
